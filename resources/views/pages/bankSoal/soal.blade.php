...
@section('content')
    ...
    <div class="col-md-4 mt-3">
        <div class="bg-white shadow-md rounded-md" style="height: fit-content">
            <div class="">
                <div class="grid grid-cols-3 gap-4 mx-auto mt-4">
                    ...
                    @endphp
                    @foreach (range(1, $jumlahKotakPaket[$paketSoal->id]) as $kotak)
                        @php
                            $soalItem = $paketSoal->daftarSoals->where('nomor_soal', $kotak)->first();
                        @endphp
                        <div class="col-span-1 mb-1">
                            <div class="bg-gray-200 hover:bg-gray-300 cursor-pointer shadow-md rounded-md text-center"
                                onclick="changeSoal({{ $paketSoal->id }}, {{ optional($soalItem)->nomor_soal ?? $kotak }})">
                                <div class="p-2 text-xs">{{ $kotak }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
